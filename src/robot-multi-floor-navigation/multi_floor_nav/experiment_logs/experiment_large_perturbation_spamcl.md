...
**论文表述提示**：`move_base` 周期性重规划导致调用次数远大于目标个数。本轮在大扰动下，前段出现大量 **500+ waypoints / 10000+ nodes** 的高开销全局规划，而后段大量收敛为 **13 / 12** 的短规划，说明全流程从大范围恢复逐步进入目标附近稳定修正阶段；若讨论平滑收益，应放到独立平滑对照实验中展开。
