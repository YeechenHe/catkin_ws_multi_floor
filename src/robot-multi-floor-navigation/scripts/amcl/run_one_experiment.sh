#!/bin/bash
# 单次初值扰动实验：启动 nav、等待就绪、发送 /start、运行固定时长、保存日志
...
#       [smoothing_enabled] [gui] [headless] [manual_lift_control]
#  method: baseline | spamcl | apamcl（模块 B） | relocc（模块 C：重定位置信度判据）
#          sp_amcl_c（模块 A + 模块 C 联合）
# 示例:
#   ./run_one_experiment.sh spamcl 0 0 0 1 180 /tmp/exp_logs 0.20 0.12 0.20 0.12 0 -1 false true false false
...
# 实验收尾阶段只优雅停机，避免 rosmaster 被 kill -9 导致“闪退”观感
cleanup_ros_graceful() {
    local pattern="gzserver|gzclient|rosmaster|roscore|multi_floor_navigation_node|move_base|map_server|change_map_node|rviz|robot_state_publisher|ekf_localization|twist_mux|twist_marker_server|urdf_spawner|controller_spawner|jackal_velocity"
    local pids
    pids=$(pgrep -f "$pattern" 2>/dev/null || true)
    for pid in $pids; do
...
SMOOTHING_ENABLED="${14:-true}"
USE_GUI="${15:-false}"
USE_HEADLESS="${16:-true}"
MANUAL_LIFT_CONTROL="${17:-false}"
...
echo "[run_one_experiment] method=$METHOD pert=($OX,$OY,$OT) sigma_L0=($SIGMA_XY_L0,$SIGMA_YAW_L0) sigma_L1=($SIGMA_XY_L1,$SIGMA_YAW_L1) K_override=$RELOC_K_OVERRIDE max_linear_error_L1=$MAX_LINEAR_ERROR_L1 smoothing=$SMOOTHING_ENABLED gui=$USE_GUI headless=$USE_HEADLESS manual_lift_control=$MANUAL_LIFT_CONTROL run=$RUN_ID duration=${DURATION}s log=$LOG_FILE"
...
(
  ...
  timeout --signal=INT "$LAUNCH_TIMEOUT_SEC" roslaunch multi_floor_nav nav.launch \
    enable_module_a:="$USE_REGION" \
    use_reference_lift_behavior:="true" \
    max_linear_error_L1:="$MAX_LINEAR_ERROR_L1" \
    init_pose_offset_x:="$OX" init_pose_offset_y:="$OY" init_pose_offset_theta:="$OT" \
    manual_lift_control:="$MANUAL_LIFT_CONTROL" \
    smoothing_enabled:="$SMOOTHING_ENABLED" \
    gui:="$USE_GUI" headless:="$USE_HEADLESS" \
    2>&1 | tee "$LOG_FILE"
) &
LAUNCH_PID=$!
...
# 优雅结束 roslaunch，并进行优雅清理
echo "[run_one_experiment] Stopping launch (SIGINT)..."
kill -INT $LAUNCH_PID 2>/dev/null || true
sleep 3
kill -9 $LAUNCH_PID 2>/dev/null || true
wait $LAUNCH_PID 2>/dev/null || true
echo "[run_one_experiment] Gracefully cleaning up residual processes after run..."
cleanup_ros_graceful
